...
set chrome_command=C:\Users\victoriest\AppData\Local\Google\Chrome\Application\chrome.exe
@REM set chrome_command=D:\chromedriver.exe
...
echo -userdir       --user-data-dir to chrome^(-userdir:custom_dir^)
echo.
...
set tagui_user_profile="tagui_user_profile"
rem check baseline parameter to output files to baseline directory
if "%arg2:~0,8%"=="-userdir" (
	set tagui_user_profile="%arg2:-userdir:=%"
	echo using profile "%arg2:-userdir:=%"
)
if "%arg3:~0,8%"=="-userdir" (
	set tagui_user_profile="%arg2:~9,%"
)
if "%arg4:~0,8%"=="-userdir" (
	set tagui_user_profile="%arg2:~9,%"
)
...
if exist "tagui_chrome.in" (
	...
	start "" "%chrome_command%" --user-data-dir="%~dp0chrome\\%tagui_user_profile%" !chrome_switches! !window_size! !headless_switch!
