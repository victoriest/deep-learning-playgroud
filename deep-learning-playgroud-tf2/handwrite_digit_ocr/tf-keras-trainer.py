...
def digit_ocr_train(enable_space=True):
    x_train, y_train, x_test, y_test = \
        load_digit_dataset('E:/_dataset/dataset_28_28_3/xbk_32.npz')

    num_classes = 10
    if enable_space:
        num_classes = 11

    model = OcrModel.get_model(ModelType.DenseNet121, num_classes)
    ...
    model_check_file_template = os.path.join("./model/dn121-{epoch:02d}-{val_loss:.2f}.h5")
    ...
    # 各种callback的ref: https://keras.io/zh/callbacks/
    lr_schedule = lambda epoch: 0.0002 * 0.9 ** epoch
    learning_rate = np.array([lr_schedule(i) for i in range(50)])
    change_learing_rate_callback = LearningRateScheduler(lambda epoch: float(learning_rate[epoch]), verbose=1)
    early_stopping_callback = EarlyStopping(monitor='val_loss', patience=3, verbose=1)
    ...
    model.fit(x_train, y_train, validation_data=(x_test, y_test),
              batch_size=192, epochs=50, verbose=1, shuffle=False,
              # validation_split=0.85,
              callbacks=[tensor_board_callback,
                         checkpoint_callback,
                         # change_learing_rate_callback,
                         early_stopping_callback])
    model.save('dn121-all-train.h5')
...
if __name__ == '__main__':
    ...
    # digit_ocr_train(enable_space=False)

    model = tf.keras.models.load_model('dn121-all-train.h5')
    model.summary()
    g = os.walk('./out_test')
    for path, dir_list, file_list in g:
        for file_name in file_list:
