...
if __name__ == "__main__":
    ...
    for num in range(500000):
        # 生成随机2-5个空白格子
        cell_count = 5
        img = np.ones((cell_h, cell_w * cell_count + 4*5, 1), np.uint8) * 255
        ...
        for i in range(digit_count):
            ...
            # print("最后一维变为单通道后的shape:", d_img.shape)
            d_img = cv2.resize(d_img.astype(float), (32, 32))
            # print("经历过cv2.resize的shape:", d_img.shape)
            tag_str += str(d_tag)
            # img_roi = img[0:cell_h, i * cell_w:((i + 1) * cell_w)]
            # print("经历过cv2.resize的shape:", img_roi.shape)
            # img_roi = cv2.bitwise_and(d_img, img_roi)
            d_w = np.random.randint(-4, 4)
            d_h = np.random.randint(-4, 4)
            M = np.float32([[1, 0, d_w], [0, 1, d_h]])
            d_img = cv2.warpAffine(d_img, M, (32, 32), borderValue=255)
            # img[0:cell_h, i * cell_w:((i + 1) * cell_w)] = d_img

            start_w = offset_w + d_w
            if start_w < 0:
                start_w = 0
            print(i, img.shape, d_w, offset_w + d_w, start_w, start_w + 32)
        ...
        img = cv2.warpAffine(img, M, (cols, rows))
        img = cv2.bitwise_not(img)
        cv2.imwrite("e:/_dataset/_digit_crnn_train_32_1/" + t_file_name, img)

        # cv2.imshow(tag_str, img)
        # cv2.waitKey(0)
